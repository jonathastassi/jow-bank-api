...
const { registerUser, loginUser, getUser, updateUser } = require('../domains/commands/users')
...
const getUserLogged = (request, response) => {
  ...
  getUser(userRepositoryInstance).call(userId)
...
const update = (request, response) => {
  const userId = request.userId
  const { name, email, password, photo, cellphone } = request.body
  const userToUpdated = { name, email, password, photo, cellphone }

  updateUser(userRepositoryInstance).call(userId)(userToUpdated)
